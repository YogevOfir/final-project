...
Clutter = np.array([0, 0, 0])  # label 0
Building_Wall = np.array([102, 102, 156]) # label 1
Road = np.array([128, 64, 128]) # label 2
Veg = np.array([107, 142, 35]) # label 3
Car = np.array([0, 0, 142]) # label 4
Building_Roof = np.array([70, 70, 70]) # label 5
Water = np.array([0, 200, 200]) # label 6
...
def label2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 0, axis=0)] = Clutter
    mask_rgb[np.all(mask_convert == 1, axis=0)] = Building_Wall
    mask_rgb[np.all(mask_convert == 2, axis=0)] = Road
    mask_rgb[np.all(mask_convert == 3, axis=0)] = Veg
    mask_rgb[np.all(mask_convert == 4, axis=0)] = Car
    mask_rgb[np.all(mask_convert == 5, axis=0)] = Building_Roof
...
def rgb2label(label):
    ...
    label_seg[np.all(label == Building_Wall, axis=-1)] = 1
    label_seg[np.all(label == Road, axis=-1)] = 2
    label_seg[np.all(label == Veg, axis=-1)] = 3
    label_seg[np.all(label == Car, axis=-1)] = 4
    label_seg[np.all(label == Building_Roof, axis=-1)] = 5
