...
def uavid2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 1, axis=0)] = [102, 102, 156]
    mask_rgb[np.all(mask_convert == 2, axis=0)] = [128, 64, 128]
    mask_rgb[np.all(mask_convert == 3, axis=0)] = [107, 142, 35]
    mask_rgb[np.all(mask_convert == 4, axis=0)] = [0, 0, 142]
    mask_rgb[np.all(mask_convert == 5, axis=0)] = [70, 70, 70]
    mask_rgb[np.all(mask_convert == 6, axis=0)] = [0, 200, 200]
    mask_rgb = cv2.cvtColor(mask_rgb, cv2.COLOR_RGB2BGR)
    return mask_rgb
...
def get_img_padded(image, patch_size):
    print(f"Input tensor shape before padding: {image.shape}")
    ...
    img_pad = pad['image']
    print(f"Output tensor shape after padding: {img_pad.shape}")
...
def main():
    ...
    for seq in seqs:
        ...
        for img_path in img_paths:
            # Start timing for this specific image inference
            ...
            with torch.no_grad():
                ...
                for input in tqdm(dataloader, desc=f"Inferencing {img_name}"):
                    # raw_predictions NxCxHxW
                    raw_predictions = model(input['img'].cuda(0))
            ...
            # Convert to RGB if dataset requires
            ...
            assert img_shape == output_mask.shape, "Output mask shape does not match input image shape."

            cv2.imwrite(os.path.join(output_path, img_name), output_mask)

        # End timing for this specific image inference
    end_time = time.time()
    inference_duration = end_time - start_time
    print(f"Inference for {seq} took {inference_duration:.4f} seconds")
